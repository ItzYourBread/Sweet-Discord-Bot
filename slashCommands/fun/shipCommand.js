...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        if (shipPercentage >= 25) {
            cAnswer = "That's not so good combo.";
        }
        if (shipPercentage >= 40) {
            cAnswer = 'Good :з';
        }
        if (shipPercentage >= 70) {
            cAnswer = 'Great combo :0';
        }
        if (shipPercentage >= 90) {
            cAnswer = "You're created for eachother! >w<";
        }
        ...
        const embed = new MessageEmbed()
            ...
            .setImage(
                `https://api.popcat.xyz/ship?user1${user1.displayAvatarURL({
                    dynamic: false,
                    format: 'png',
                })}&user2=${user2.displayAvatarURL({
                    dynamic: false,
                    format: 'png',
                })}`
            )
            .setDescription(
                `<:arrowdown:976450407243083827> \`${user1.username}\`\n<:arrowup:976450449760739358> \`${user2.username}\`\n\n**Loveing ${shipPercentage}%** ${cAnswer}`
            );

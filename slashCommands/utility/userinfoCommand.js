...
module.exports = {
    ...
    if (!findUser) {
      ...
      user = newUser;
        ...
        if (Target?.presence?.status === 'online') TargetStatus = '<:Online:974780062811455528> Online';
        if (Target?.presence?.status === 'idle') TargetStatus = '<:Idle:974780108470616064> Idle';
        if (Target?.presence?.status === 'dnd') TargetStatus = '<:DoNotDisturb:974780209540771890> Do Not Disturb';
        if (!Target?.presence?.status || Target?.presence?.status === 'undefined') TargetStatus = '<:Offline:974780153857208330> Offline';

        
      if (user && user.isPremium)
TargetPremium = 'Activated';
      if (user && !user.isPremium)
TargetPremium = 'Poor';

...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!command) return interaction.reply({ content: 'Awh its a huge error notify our devoloper\nhttps://discord.gg/KBgdDG9TPc' });
        
        if (command.ownerOnly) {
            if (interaction.user.id !== admin) {
                return interaction.reply({ content: `This command only can be use by Sweet Admin(s)`, ephemeral: true });
            }
        }
        ...
        if (!interaction.member.permissions.has(command.userPerm || [])) {
            return interaction.reply({
                content: `${client.config.fail} You need \`${command.userPerm.join(", ")}\` Permissions`, ephemeral: true
            })
        }

        if (!interaction.guild.me.permissions.has(command.botPerm || [])) {
            return interaction.reply({
                content: `${client.config.fail} I need \`${command.botPerm.join(", ")}\` Permissions`, ephemeral: true
            })
        }
